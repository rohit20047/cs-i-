...
const useChatLogic = () => {
  ...
  const handleSendMessage = async (messageText, isInitial = false) => {
    try {
      ...
      // Check if user is confirming they want technical support
      const lastMessage = messages[messages.length - 1];
      const isConfirmingSupport = 
        lastMessage && 
        lastMessage.sender === "ai" && 
        (lastMessage.text.includes("Would you like to speak with a technical support specialist") ||
         lastMessage.text.includes("Do you want to connect with") ||
         lastMessage.text.includes("need to speak with")) &&
        /yes|yeah|sure|ok|okay|connect|support|please|help|specialist|tech|technical/i.test(messageText);
  
      if (isConfirmingSupport) {
        // User confirmed they want technical support
        // Show typing indicator for 2 seconds
        await showLoadingIndicator(2000);
      ...
      // Show typing indicator while processing normal message
      // await showLoadingIndicator(2000);
      setShowTypingIndicator(true);
      ...
      if (data.type === "TECHNICAL_WITH_DOCS") {
      ...
      // Show product options if available and not in email input step
      if (data.showOptions && currentStep !== "emailInput") {
        setShowOptions(true);
      } else {
        setShowOptions(false);
      }
